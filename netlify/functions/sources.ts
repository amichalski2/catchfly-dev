...
import { sourceHealth } from './lib/product-store.ts';
import { projectExists } from './lib/store.ts';

export const config = { path: '/api/projects/:projectId/sources' };
...
export default async function handler(
  ...
  return json(200, { sources: await sourceHealth(projectId) });
}
